...
class TimeRow extends Component {
  ...
  renderColumn(){
    return this.props.durations.map(duration => <div className="center aligned column" key={duration.start}>{`${duration.start}-${duration.end}`}</div>)
  }
  ...
  render(){
    ...
    return (
      <div className="sixteen column row">
        <div className="center aligned column">2/61</div>
        {this.renderColumn()}
      </div>
    );
  }
